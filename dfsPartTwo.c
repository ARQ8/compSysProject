...
void createChild(int PN, int nums[], int start, int seg, int fd[][2], bool finalProcess, int H, int size) {
   ...
   if (pid < 0) {
       ...
       if (PN == 1) {
           ...
           write(fd[0][WRITE_END], &keysInProc, sizeof(keysInProc));
           printf("Child PID: %d\n", getpid());
       ...
       read(fd[PN-1][READ_END], &childKeys, sizeof(childKeys));
       
        int newCount = findKey(nums, start, end, H, count,PN);
       ...
       if (finalProcess == true) {
        ...
        fclose(file);
        int keysInProc = newCount-count;
        if(count>2){
            count=2;
        }
       write(fd[PN][WRITE_END], &totalAvgSum, sizeof(totalAvgSum));
       write(fd[PN][WRITE_END], &maxChild, sizeof(maxChild));
       write(fd[PN][WRITE_END], &newCount, sizeof(newCount));
       write(fd[PN][WRITE_END], &keysInProc, sizeof(keysInProc));
       raise(SIGTSTP);
