...
void printHiddenKeys(int arr[], int start, int end) {
    for (int i = start; i <= end; i++) {
        if (arr[i] < 0) {
            printf("The hidden key is %d in position %d\n", arr[i], i);
        }
    }
}
void create_processes(int n, int start, int seg, int num[]) {
    ...
    if (child1 == 0) { // Child process
        // Code before modification remains the same
        printf("Child %d created with parent %d\n", getpid(), getppid());

        // Calculate local max, average, and print hidden keys for the first child's segment
        int max = findMax(num, start + seg, start + (2 * seg) - 1);
        float avg = findAverage(num, start + seg, start + (2 * seg) - 1);
        printf("Local Max: %d, Local Avg: %f\n", max, avg);
        printHiddenKeys(num, start + seg, start + (2 * seg) - 1);

        // Recursive call and exit remain unchanged
        start = start + (3 * seg);
        ...
        if (child2 == 0) { // Second child
            // Calculate local max, average, and print hidden keys for the second child's segment
            printf("Child %d created with parent %d\n", getpid(), getppid());
            int max = findMax(num, start, start + seg - 1);
            float avg = findAverage(num, start, start + seg - 1);
            printf("Local Max: %d, Local Avg: %f\n", max, avg);
            printHiddenKeys(num, start, start + seg - 1);
        ...
        // Parent process now waits for both children and then exits
        int status;
